...
let user=ref({})
let blogs=ref([])
let blogs2=ref([])// 关注的人的播客
let isEdit=ref(false)
let minTime=ref(0) // 上一次拉取到的时间戳
   let offset=ref(0)// 偏移量
let count=ref(5) // 总数
let isReachBottom=ref(false) // 是否到底了
let userId=null;
if (users) {
...
function   queryBlogsOfFollow(clear) {
  ...
  http.get("blog/of/follow", {
    params: {offset: os, lastId: minTime.value ||
          new Date().getTime() + 1}
  }).then(({data}) => {
        if (!data) {
          return;
        ...
        blogs2.value = clear ? list : blogs2.concat(list);
        params = params;
      })
      .catch(e => console.log(e))
}
...
function toEdit() {
  console.log(userId)
  r.push('info-edit', { params: { userId: userId } });
}
function logout() {
...
const queryBlogById = async(b) => {
  const res = await queryBlogById(b.id);
  console.log(res);
  b.liked = res.liked;
  b.isLike = res.isLike;
}
        ...
        <el-tab-pane label="笔记" name="1">
          <div v-for="b in blogs" :key="b.id" class="blog-item">
            ...
            <div class="blog-info">
              ...
              <div class="blog-liked"><img src="@/assets/images/thumbup.png" alt=""> {{b.liked}}</div>
        ...
        <el-tab-pane label="关注(0)" name="4">
          <div class="blog-list" @scroll="onScroll">
            <div class="blog-box" v-for="b in blogs2" :key="b.id">
              ...
              <div class="blog-title">{{b.title}}</div>
              <div class="blog-foot">
                <div class="blog-user-icon"><img :src="b.icon || '/images/default-icon.png'" alt=""></div>
