...
import {getVoucherByResortId, queryHotelByIdAPI, queryResortByIdAPI, queryShopByIdAPI} from "@/api/layout.js";
...
const query = async(id) => {
  const res = await queryShopByIdAPI(id);
  console.log(res);
  shops.value = res;
  if (res.images) {
    shops.value.images = shops.value.images.split(",")
  }
}
query(rid);
  ...
  <div class="background">
    ...
    <div class="container">
      ...
      <div class="shop-rank">
        ...
        <span>总评分：{{shops.score}}</span>
        <div><i class="el-icon-arrow-right"></i></div>
        <span v-if="shops.contact">联系方式：{{shops.contact}}</span>
        <span v-if="shops.title">{{shops.title}}</span>
        <span v-if="shops.avgPrice">人均：{{shops.avgPrice}}</span>
<span v-if="shops.sold">销量：{{shops.sold}}</span>
      </div>
      <div class="shop-address">
        <div><i class="el-icon-map-location"></i></div>
        <span>{{shops.area}}</span>
      ...
      <div class="shop-images">
        <div v-for="(s,$index) in shops.images" :key="$index" >
      ...
      <el-carousel v-show="showImageIsShow" class="img-right" >
        ...
        <el-carousel-item v-for="(s,i) in shops.images" :key="i">
          <img :src="s" alt="" class="carousel-img">
        </el-carousel-item>
      </el-carousel>

      <div class="shop-detail">
        营业时间：{{shops.openHours}}<br>
      </div>
      ...
      <div class="shop-comments">
        <div class="comments-head">
          <div>网友评价 <span>{{shops.comments}}</span></div>
          <div><i class="el-icon-arrow-right"></i></div>
        </div>
...
.background {
  ...
  //overflow: auto; /* 添加滚动条 */
  background-image: url('https://p4.itc.cn/q_70/images03/20200805/627942714e944674a3e1ef41f0680ac2.png');
  background-size: 100%;
