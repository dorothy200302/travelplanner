...
import {getVoucherByHotelId, queryHotelByIdAPI, queryHotelTypeAPI} from "@/api/layout.js";
import axios from "axios";
import http from "@/utils/http.js";
import ItemSearch from "@/components/itemSearch.vue";

let r=useRouter();
let hid=r.currentRoute.value.params.id;
let id=sessionStorage.getItem("id");
let hotel=ref({});
let vouchers=ref([]);let users=sessionStorage.getItem("user");
let userId=null;
if (users) {
  const user = JSON.parse(users);
  userId = user.userId;
}
const queryVoucher = async(id) => {
  const res = await getVoucherByHotelId(id);
  console.log(res);
  vouchers.value = res;
  }
queryVoucher(hid);
const queryHotel = async(id) => {
  const res = await queryHotelByIdAPI(id);
  console.log(res);
  hotel.value = res;
hotel.value.picture=hotel.value.picture.split(",")}
queryHotel(hid);
console.log(hid)
...
function seckill(v){
  ...
  // 秒杀抢购
  http.post("hotel/VoucherSecKill" + cacheEntity)
      .then(({data}) => {
    ...
    <div class="container">
     <h1>{{hotel.name}}</h1>
      ...
      <div class="shop-rank">
        ...
        <span>总评分：{{hotel.comment}}</span>
        <div><i class="el-icon-arrow-right"></i></div>
      </div>
      <div class="shop-address" style="color:black">
        ...
        <div style="margin: 0 15px;color:black;font-size: 16px;padding: 40px 20px;border-bottom: 1px solid #e1e2e3;">
{{hotel.description}}
        </div>
      </div>
      <div class="shop-images">
        <div v-for="s in hotel.picture" :key="index" >
...
<!--          <el-carousel-item v-for="(s,i) in hotel.picture" :key="i">-->
<!--            <img :src="s" alt="" class="carousel-img">-->
<!--          </el-carousel-item>-->
<!--        </el-carousel>-->
      </div>
      <div class="shop-detail" style="color: black;padding: 40px 20px">
基础设施：{{hotel.infrastructure}}<br>
      </div>
      <div class="shop-detail" style="padding: 40px 20px;color: black;">
        入住须知:{{hotel.notice}}
      </div>
...
.background {
  ...
  overflow: auto; /* 添加滚动条 */
  background-image: url('https://dimg04.c-ctrip.com/images/0202m120009qzzged8F75_W_1080_808_R5_D.jpg');
  background-size: 100%;
...
.shop-address {
  ...
  color: #82848a;
  height: 72px;
  display: flex;  overflow: auto; /* 添加滚动条 */
